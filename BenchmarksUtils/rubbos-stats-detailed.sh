...
echo "STARTING"
echo "Timestamp Start;Timestamp End;Run #;Node;Clients;Average Throughput (req/s);Average Response Time (ms);CPU Utilization (%);Tot Clients;Tot Avg X;Tot Avg R;Tot Avg U;CPU Demand;"  > $reportOut
...
do
	...
	do
		...
		#Stampo la riga della run client $i
		echo "$tsStart;$tsEnd;$runNumber;$i;$clientsNumber;$X;$R;$U;"  >> $reportOut
	done

	#Stampo la riga complessiva della run $runNumber
	echo ";;$runNumber;;;;;;$(($clientsNumber * 2));$totX;$(($totR / $numClientNodes));`echo "$totU / $numClientNodes" | bc -l`;`echo "$totU / $totX" | bc -l`;"  >> $reportOut
