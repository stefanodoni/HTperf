...
class SUTConfig:
    ...
    def set(self, system_config):
        ...
        self.CPU_NOMINAL_FREQUENCY = system_config['BaseFrequency'] * 1000000000 # The one advertised in CPU Model Name
        ...
        if int(system_config['TurboBoost']) == 0: # TB ON
            ...
            self.CPU_MAX_FREQUENCY_ALL_CORES_BUSY = self.CPU_NOMINAL_FREQUENCY

        # If the TSC is different from what is advertised and TB ON, we use the CPU_BASE_OPERATING_FREQUENCY as CPU_NOMINAL_FREQUENCY
        if self.CPU_BASE_OPERATING_FREQUENCY != self.CPU_NOMINAL_FREQUENCY: # and int(system_config['TurboBoost']) == 0:
            print("Warning: frequencies are not equal! CPU_BASE_OPERATING_FREQUENCY: " + str(self.CPU_BASE_OPERATING_FREQUENCY) + " while CPU_NOMINAL_FREQUENCY: " + str(self.CPU_NOMINAL_FREQUENCY))
            print("Setting CPU_NOMINAL_FREQUENCY equals to CPU_BASE_OPERATING_FREQUENCY")
            self.CPU_NOMINAL_FREQUENCY = self.CPU_BASE_OPERATING_FREQUENCY

        print("CPU_NOMINAL_FREQUENCY: " + str(self.CPU_NOMINAL_FREQUENCY))
    ...
    def set_manual(self):
        ...
        self.CPU_NOMINAL_FREQUENCY = 2500000000 # That is the CPU Ref TSC
